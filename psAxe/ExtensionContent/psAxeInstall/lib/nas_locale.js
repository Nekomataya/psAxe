...
//localizeメソッドに置きかえ機能を付ける
	nas.localize=function(myObject,myArg){
		if(typeof myObject != "string"){
			if( ! myArg ){myArg=[];}
			if(!(myArg instanceof Array)){myArg=[myArg];}
			var myResult=myObject[nas.locale];
			if(myArg.length>0){
			 // 言語リソース内の%d(1…)を置き換える　myArgはstring要素の配列でなくてはならない
				for (var rId=0;rId<myArg.length;rId++){var myRegex=new RegExp("%"+(rId+1),"g");myResult=myResult.replace(myRegex,myArg[rId]);}
			}
			return myResult;
		}else{
