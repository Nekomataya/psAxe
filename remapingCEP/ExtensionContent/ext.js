...
//	var jquery = requier('./node_modules/jquery/index.js');
//	var jquery-ui = requier('jquery-ui');
};
/*
 *		スタートアップ処理
 */
...
doScript('app.nas.Version.psAxePman="psAxePman:'+extVer+'";');
//バージョントレーラーにエクステンションバージョンを設定
//	chgTab("PTHhandle");//tab初期化　本体用
//	chgPnl("3");//toolbarパネル初期化
	applyHostLocale();//locale取得して反映
	if(extVer>="1.1.0") applyHostProp();//プロパティ初期化
	if((nas.Console)&&(document.getElementById('console'))){
//nas-consoele初期化
//alert('setup-nas.PS.console')
 ...
 *	@params {String}	cmd
 *		実行コマンド
 *	@params	{Function}	cback
 *		callback function arguments
 *
 *	CSX|CEP用のアプリケーションエンジンでESTKコマンドを実行してリザルトをコールバックに渡す
 *	CS6 CC 汎用 ext.jsに設定
 *
 */
