...
//内部で使用する  簡易localizeファンクションを作っておく。PS/AE等で利用する限りこのルーチンは使用しない
...
ja:"ぎゃー! なんか想定外のことが起きたヨ\n=================\n %1 \n======================\n上のメッセージをねこまたやまでお知らせくださると、なんとかなるかも知れません。\nダメならあきらめてチョ"};
    ...
    myInstall.uiMsg.skipMkDir	={en:"folder %1 already exists. skip mkdir",ja:"フォルダ %1 :がすでにあるっぽいのでパス"};
    ...
    myInstall.uiMsg.cannotRemoveDir	={en:"Folder %1 not exists. cannot remove",ja:"フォルダ %1 は無い様"};
...
if(checkNG){
	alert(myInstall.localize(myInstall.uiMsg.tryReInstall));
}else{
/*
...
nas_Installer.abortProcess	=function(msg){
	alert(myInstall.localize(myInstall.uiMsg.unexpctedErrorOccurred,msg));
//"ぎゃー! なんか想定外のことが起きたヨ\n=================\n%MSG%\n======================\n上のメッセージをねこまたやまでお知らせくださると、なんとかなるかも知れません。\nダメならあきらめてチョ"
	this.pushLog(myInstall.localize(myInstall.uiMsg.unexpctedError)+msg);//"想定外エラー :"
...
case	"makeFolder":
		...
		if(! targetDir.exists){
			...
			this.showMsg(localize(myInstall.uiMsg.skipMkDir,targetDir.fsName));//"フォルダ %TARGET% がすでにあるっぽいのでパス"
...
//フォルダコンテンツ確認
			...
			this.showMsg(localize(myInstall.uiMsg.cannotRemoveDir,targetDir.fsName) );//"フォルダ %TARGET% は無い様"
