...
nas.GUI.addStaticText(nas.Pref.tabPanel[3],nas.localize(nas.uiMsg["atCreateNewLayer"]),1,1.5,3,1).justify="right";//"新規レイヤ作成時に"
nas.Pref.tabPanel[3].nlOpc=nas.GUI.addCheckBox(nas.Pref.tabPanel[3],nas.localize(nas.uiMsg["withTransparent"]),4,1.5,2,1);//"透過させる"
...
//=========色選択
//動画レイヤ
nas.Pref.tabPanel[3].colorSPC=nas.GUI.addPanel(nas.Pref.tabPanel[3],nas.localize(nas.uiMsg["newLayerBgColor"]),2,2,6,2);//"新規レイヤの背景色"
...
//修正レイヤ
nas.Pref.tabPanel[3].colorSPCo=nas.GUI.addPanel(nas.Pref.tabPanel[3],nas.localize(nas.uiMsg["overlayBgColor"]),2,3.7,6,2);//"修正レイヤの背景色"
...
//プレビューコントロール設定
	nas.GUI.addStaticText(nas.Pref.tabPanel[3],nas.localize(nas.uiMsg["previewControl"]),0,7,3,1).justify="right";//"プレビューコントロール"

nas.Pref.tabPanel[3].focusInterlocking=nas.GUI.addCheckBox(nas.Pref.tabPanel[3],nas.localize(nas.uiMsg["dm002"]),4,6,6,2);//"フレーム移動時にアクティブレイヤを移動"
nas.Pref.tabPanel[3].playheadMoveToOptKey=nas.GUI.addCheckBox(nas.Pref.tabPanel[3],nas.localize(nas.uiMsg["dm010"]),4,7,6,2);//"ヘッド移動に不透明度キーを使用(timeline)"
nas.Pref.tabPanel[3].playheadSkipFrames=nas.GUI.addEditText(nas.Pref.tabPanel[3],"0",4,8.5,1,1);
nas.Pref.tabPanel[3].playheadSkip=nas.GUI.addStaticText(nas.Pref.tabPanel[3],nas.localize(nas.uiMsg["dm011"]),5,8.5,5,2);//"frame skip (timeline)"

//ドキュメント設定
	nas.GUI.addStaticText(nas.Pref.tabPanel[3],nas.localize(nas.uiMsg["Documents"]),0,11,3,1).justify="right";//"ドキュメント設定"

//nas.GUI.addStaticText(nas.Pref.tabPanel[3],"新規ファイル作成ダイアログ",1,9,2,1).justify="right";
nas.Pref.tabPanel[3].selectDialog=nas.GUI.addCheckBox(nas.Pref.tabPanel[3],nas.localize(nas.uiMsg["dm004"]),4,10,6,1);//"新規ドキュメント作成時にアニメ拡張機能"
//タップ画像配置時にブレンドモードを差の絶対値にする
nas.Pref.tabPanel[3].pegBlendMode=nas.GUI.addCheckBox(nas.Pref.tabPanel[3],nas.localize(nas.uiMsg["dm013"]),4,11,6,1);//"タップ画像配置時に差の絶対値にする"
//フレーム画像配置時に２０％透過にする
nas.Pref.tabPanel[3].frameOpacity=nas.GUI.addCheckBox(nas.Pref.tabPanel[3],nas.localize(nas.uiMsg["dm014"]),4,12,6,1);//"フレーム画像配置時に半透明にする"

//ショートカット用スクリプトインストール
	nas.GUI.addStaticText(nas.Pref.tabPanel[3],nas.localize(nas.uiMsg["shortcutKey"]),0,14,3,1).justify="right";//"ショートカット用ツール"

	nas.Pref.tabPanel[3].utInstall	=nas.GUI.addRadioButton(nas.Pref.tabPanel[3],nas.localize(nas.uiMsg["install"]),4,14,2,1);//"インストール"
	nas.Pref.tabPanel[3].utRemove	=nas.GUI.addRadioButton(nas.Pref.tabPanel[3],nas.localize(nas.uiMsg["uninstall"]),6,14,2.5,1);//"アンインストール"
...
//TitleDB
	...
	{
		if(this.parent.list.options.length==1) {alert(nas.localize({
	en:"It can not be deleted because the entry is 0.",
	ja:"エントリが0になるので削除できません。"
		}));return;}
		if(confirm(nas.localize({
	en:"It will delete the selected entry.\n Are you sure?",
	ja:"選択されたエントリを削除します。\nよろしいですか?"
		})))
		{
...
//ターゲットフォルダが無ければ作る
	...
	ja:"フォルダを作成しました"
		...
		alert(nas.localize({
	en:"you can not create a folder. It must be executed by a user with administrative privileges.",
	ja:"フォルダの作成ができません　管理者権限のあるユーザで実行して下さい"
		}));doCopy=false;}
        }
if(doCopy){
//コピー
			for(ix in myTargets){
				if(! File(myTargets[ix]).exists)
				{
					...
					var msg=names.join(" ,")+nas.GUI.LineFeed+counter +nas.localize({
	en:" of the shortcut script has been installed. Please setup the shortcut, photoshop after restart",
	ja:" 個の　ショートカット用スクリプトをインストールしました。PSを再起動後にショートカットの設定をして下さい"
					});
					alert(msg);
}
		}else{
//削除　nasフォルダは残置?
			for(ix in myTargets){
				if(File(myTargets[ix]).exists)
				{
					...
					var msg=names.join(" ,")+nas.GUI.LineFeed+counter +nas.localize({
						en:" of the shortcut script has been removed.",
						ja:" 個の　ショートカット用スクリプトを削除しました。"
					});
					alert(msg);
		}
	}	
...
//Yes/No/Cacel
	...
	nas.Pref.writeButton.onClick=function(){
		var doAction=confirm(nas.localize({
	en:"The setting [%FOLDER%] you will save below. \nPreviously the file will be overwritten. Are you sure?",
	ja:"設定を[%FOLDER%]以下に保存します。\n以前のファイルは上書きされます。よろしいですか？"
	}).replace(/%FOLDER%/,nas.prefarenceFolder.fsName));
